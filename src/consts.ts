// Place any global data in this file.
// You can import this data from anywhere in your site by using the `import` keyword.

export const SITE_TITLE = 'QuCS - Quantum Computing Seminar'
export const SITE_DESCRIPTION =
	'Building basic quantum knowledge and introducing latest research for audiences without quantum computing background but interested in learning'

// If you deploy to a subdirectory, set the `WEBSITE_BASE` here.
// e.g. '/blog' if you are deploying to example.com/blog
// Leave it as an empty string if you are deploying to a root domain (e.g. example.com)
export const SITE_BASE = ''

export const SocialLinks = [
	{
		name: 'GitHub',
		url: 'https://github.com/yashjawale',
	},
	{
		name: 'LinkedIn',
		url: 'https://www.linkedin.com/in/yashjawale/',
	},
	{
		name: 'X / Twitter',
		url: 'https://twitter.com/imyashjawale',
	},
	{
		name: 'Instagram',
		url: 'https://instagram.com/imyashjawale',
	},
]

export const WebsiteLinks = [
	{
		name: 'Home',
		url: '',
	},
	{
		name: 'Lectures',
		url: 'lectures',
	},
	{
		name: 'Community',
		url: 'community',
	},
	{
		name: 'About',
		url: 'about',
	},
]
